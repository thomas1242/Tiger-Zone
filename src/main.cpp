...
int main() {
    ...
    // Start the game loop
    while (window.isOpen() && game->status() == ON)
    {
        // Process events
        sf::Event event;
...
//                window.draw( deckTopTile );
        ...
        // Draw the string
        string score = int2Str( game->getBoard()->getDeck()->getSize() ) + " cards left in the deck,\nnot including current card";
        ...
        for(int i = 0; i < 11; i++) {
            for(int j = 0; j<11; j++) {
                if( game->getBoard()->getCard(i, j).getId() > -1 ) {
                    tiles[ game->getBoard()->getCard(i, j).getId() + 1 ] .setPosition(j * (size + 1) + 10 + offset, i * (size + 1) + 10 + offset);
                    tiles[ game->getBoard()->getCard(i, j).getId() + 1 ] .setOrigin((size/2), (size/2));
                    tiles[ game->getBoard()->getCard(i, j).getId() + 1 ] .rotate( 360 - rots[game->getBoard()->getCard(i, j).getId()] );
                    rots[game->getBoard()->getCard(i, j).getId() ] = game->getBoard()->getCard(i, j).getOrient();   // set rot
                    tiles[ game->getBoard()->getCard(i, j).getId() + 1 ] .rotate( game->getBoard()->getCard(i, j).getOrient() );
